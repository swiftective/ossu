...
(check-expect (solve BD5) BD5s) ; Tests
...
(define (next-boards bd)
  (filter valid? (generate-boards bd)))
...
(check-expect (valid? BD3) true)
(check-expect (valid? BD4) true)
(check-expect (valid? BD5) true)
(check-expect (valid?
                (cons 2 (rest BD2))) false)
(check-expect (valid?
                (cons 2 (rest BD3))) false)
(check-expect (valid? (fill-square BD4 1 6)) false)


;; (define (valid? bd) true) ;stub
...
;; Board -> (listof Board)
;; find the first one in the list that is false and generate 9 boards
;; placing numbers 1-9 in that position

(check-expect (generate-boards (list 1 false))
              (list (list 1 1)
                    (list 1 2)
                    (list 1 3)
                    (list 1 4)
...
(check-expect (generate-boards BD1)
...
;; Board -> Pos
;; find the position of false in the Board

(check-expect (find-false (list 1 2 false)) 2)
(check-expect (find-false (list 1 false false)) 1)
(check-expect (find-false (list false 2 3)) 0)


;; (define (find-false bd) 10) ;stub

(define (find-false bd)
  (cond
    [(empty? bd) (error "The board does not contain false")]
    [else
     (if (false? (first bd))
         0
         (add1 (find-false (rest bd))))]))
