...
(define (make-point _x _y)
  (obj
   ...
   (list (cons 'get-x (λ (self args) (get self 'x)))
         ...
         (cons 'set-x (λ (self args) (set self 'x (car args))))
         (cons 'set-y (λ (self args) (set self 'y (car args))))
         (cons 'distToOrigin
               (λ (self args)
  ...
  (let ([pt (make-point _x _y)])
    (obj
     (cons (mcons 'color _c)
           (obj-fields pt))
     (append (list
              (cons 'get-color (λ (self args) (get self 'color)))
              (cons 'set-color (λ (self args) (set self 'color (car args)))))
             (obj-methods pt)))))
    ...
    (obj
     ...
     (append ; overriding by being earlier in the list (see send function)
      (list
       ...
       (cons 'get-x (λ (self args)
                      (let ([r (get self 'r)]
                            [theta (get self 'theta)])
                        (* r (cos theta)))))
       (cons 'get-y (λ (self args)
                      (let ([r (get self 'r)]
                            [theta (get self 'theta)])
                        (* r (sin theta)))))
