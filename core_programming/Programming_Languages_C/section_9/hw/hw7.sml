       ...
       let
        ...
        in
          ...
          (* let segment a start at or to the left of start of segment b *)
            let
              val ((aXstart,aYstart,aXend,aYend), (bXstart,bYstart,bXend,bYend)) =
...
fun eval_prog (e,env) =
  let
    fun shift_helper (deltaX, deltaY, e) =
      case e of
         NoPoints => e
        ...
        | _ => raise Impossible "shift_helper: Not a geometry value"
  in
      case e of
          NoPoints => e (* first 5 cases are all values, so no computation *)
              ...
              | Shift (deltaX, deltaY, exp) => shift_helper (deltaX, deltaY, eval_prog (exp, env))
  end
